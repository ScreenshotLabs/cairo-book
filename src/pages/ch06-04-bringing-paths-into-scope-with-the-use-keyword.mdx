...
<span className="caption">Listing 6-5: Bringing a module into scope with
`use`</span>
...
<span className="caption">Listing 6-6: A `use` statement only applies in the scope
it’s in</span>
...
<span className="caption">Listing 6-7: Bringing the `add_to_waitlist` function
...
<span className="caption">Listing 6-8: Bringing `ArrayTrait` into scope in an
idiomatic way</span>
...
<span className="caption">Listing 6-9: Renaming a trait when it’s brought into
...
<span className="caption">Listing 6-10: Making a name available for any code to use
